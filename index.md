...
I will not go in-depth with all aspects but I want to provide a top-level architecture which allows being extended by other architecture patterns for different needs.
...
When building your own app it is very useful to build your own little UI library whieh i call "Pure UI" from now on.
...
Wellknown style guides for web frontend are Styleguidist or StoryBook.
...
In every app, there might be UI elements that are definately only visible once on the screen like a page and some that are definately visible multiple times on the same screen.
